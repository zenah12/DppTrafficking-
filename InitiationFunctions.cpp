...
void Initiate_Profile_SS(double mat[numx], int index){
    ...
    // old
    //value = -phi*exp(2.0*L_P/lambdaexp)/(exp(1.0/lambdaexp)-1.0)/(1.0-exp(2.0*L_P/lambdaexp));
    switch (index) {
        case 0: //L
        ...
        case 1: //SR
            ...
            //mat[i] =  value*exp(-i*dx/lambdaexp)*(0.5*(1.0+exp(1.0/lambdaexp))*kon*koff*koff*(1.0+ko)-(d0*(exp(1.0/lambdaexp)+exp(-1.0/lambdaexp)-2.0)-kon*koff)*(k+(1.0+ko)*(-k-koff)))/((-1.0+exp(1.0/lambdaexp))*koff*(k*ko+(1+ko)*koff));
            break;
        case 2: //SL
        ...
        case 3: //SE
            ...
            //mat[i] =  value*exp(-i*dx/lambdaexp)*(k*kon*koff*(1.0+exp(-1.0/lambdaexp)))/(2.0*(k*ko+(1.0+ko)*koff));
            break;
        default: //SI
            for (i=0; i<numx; i++) mat[i] = (ki/k2)*value*exp(-i*dx/lambdaexp)*koff*kon*k*(1.0+exp(-1.0/lambdaexp))/2.0/con1;
            //mat[i] = (value*exp(-i*dx/lambdaexp)*(k*kon*koff*(1.0+exp(-1.0/lambdaexp)))/(2.0*(k*ko+(1.0+ko)*koff)))*ki/k2;
            break;
    }
}
