...
// ********************************************************************
// Here all the functions used in the object Dynamics are defined
// This object deals with solving the dynamical equations for steady state, and bleaching for FRAP
// ********************************************************************
...
void Dynamics::UpdateProfiles(double u_L[numx], double u_SR[numx], double u_SL[numx], double u_SE[numx], double u_Si[numx], double source[numx]){
    ...
    for(i=1; i<(numx-1); i++){
        // upadate uL pool
        u_L[i] = uL_1[i] + dt*(d0*(uL_1[i-1]-2.0*uL_1[i]+uL_1[i+1])/(dx*dx) +koff*(uSR_1[i] + uSL_1[i+1]) - kon*koff*uL_1[i] -kl*uL_1[i] + nu*source[i]);
...
// get the numerical value for the extracellular fraction using the pools at position 0
double Dynamics::getRhoNum(double u_L[numx], double u_SR[numx], double u_SL[numx], double u_SE[numx], double u_Si[numx]){
    
    double var = 0.0;
    
    var = (u_L[0]+u_SR[0]+u_SL[0])/(u_L[0]+u_SR[0]+u_SL[0]+u_SE[0]+u_Si[0]);
